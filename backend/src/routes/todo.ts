...
router.get('/', async (req: Request, res: Response) => {
 try {
  const sql = 'SELECT id, title, completed, created_at AS createdAt, updated_at AS updateAt FROM todos WHERE user_id = ? ORDER BY createdAt DESC';
...
router.put('/:id', async (req: Request, res: Response) => {
 ...
 try {
   const sql = 'UPDATE todos SET title = ?, completed = ?, updated_at = ? WHERE id = ? AND user_id  = ?';
   const params = [title, completed,new Date(), req.params.id, (req as any).user.id];
